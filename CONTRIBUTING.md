# Contributing to ep_who_did_what

Thanks for helping improve `ep_who_did_what` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_who_did_what` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_who_did_what from your local clone
pnpm run plugins i --path ../ep_who_did_what

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_who_did_what/` are picked up after a server restart.
...
* Etherpad version, ep_who_did_what version, browser + OS
* Steps to reproduce
