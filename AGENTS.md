...
`ep_who_did_what` runs inside Etherpad's test harness. From an etherpad checkout that has installed this plugin via `pnpm run plugins i --path ../ep_who_did_what`:
...
* Server: `eejsBlock_timesliderScripts`, `eejsBlock_timesliderEditbarRight`, `eejsBlock_timesliderBody`, `clientVars`, `expressCreateServer`
